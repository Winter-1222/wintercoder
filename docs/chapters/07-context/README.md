...
当前完成了四步：**单个大结果落盘**、**活动上下文视图**、**手动 `/compact`**和**自动压缩触发器**。超长请求重试和连续失败暂停还没有实现。
...
这一小步只在预算过线时主动尝试一次。供应商直接返回 prompt-too-long 后怎样压缩重试，以及连续 3 次自动压缩失败后怎样暂停，留给后续两个独立闭环。
...
没有。目前解决了单个大结果、同一任务中的旧工具结果，以及普通对话的手动和预算触发压缩。供应商直接拒绝超长请求时还不会自动压缩后重试，连续失败暂停也尚未实现。
