...
目前已经完成最小循环、自然停止、50 轮上限、界面轮次展示、用户取消、连续异常工具保护和安全工具分批并发。Plan Mode 还没有实现。
...
1. `src/jixue/agent.py`：先看 `Agent.run()`，再看 `_partition_tool_calls()`、`cancel()` 和 `_stream_llm()`。
...
2. 如果模型第一轮同时请求两个文件，两个 `read_file` 会并发执行，最后通常仍显示“共 2 轮”。
...
8. 异常工具名很难让真实模型稳定复现，可运行下面的定向测试：

```powershell
conda run --no-capture-output -n mycoder pytest tests/bridge/test_agent_loop.py -k missing -v
```

预期看到 `1 passed`。这个测试让模型客户端连续三轮请求 `not_exists`，并检查没有出现第 4 轮。
