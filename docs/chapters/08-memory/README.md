...
| `apps/desktop/src/renderer/src/App.tsx`、`state.ts` | 侧栏、界面回放、工具卡片排序与折叠、权限按钮和状态恢复 |
...
桌面显示顺序为“用户消息 → 本轮工具卡片 → 回复内容”。`tool_received` 按 requestId 把卡片插入同轮回复前，结果返回后更新原卡片；会话回放也经过同一 reducer。执行中和等待确认时展开，完成、失败后默认折叠，点击标题或用键盘可重新展开。长输入在卡片内滚动，长结果换行并限制高度；这些只是显示规则，不改模型消息或 JSONL 事件顺序。
...
10. 缩小到最小窗口，读取含长行的文件：工具卡片应位于回复上方且完成后折叠；展开后长文本在内部换行或滚动，不撑出窗口。执行写工具时确认按钮应直接可见，允许、拒绝或停止后检查折叠和重启恢复。
...
node tests/ui/tool_cards_electron.mjs
```
...
工具卡片修复验证通过：9 项前端状态测试、TypeScript 类型检查、桌面构建、工具卡片 Electron 专项与第八章 Electron 回归。专项在 860×620 最小窗口验证长输入内部滚动、长结果换行、同轮卡片顺序、完成/失败后折叠、键盘展开、允许/拒绝/停止和完整进程重启。使用临时项目和 FakeLLM，不调用真实模型。
...
- 第九章开始前修复工具卡片长内容溢出，调整为同轮回复上方显示，执行结束默认折叠；本次修复独立提交，与第九章保持边界。
